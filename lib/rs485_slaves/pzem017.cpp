...
PZEM017::status_t PZEM017::request_all(measurement_t &output) {
  ...
  if (!mbm->receive_response(response_buf, 21, 1000))
    return Timeout;
  PZEM017::status_t status = validate_response(21, READ_INPUT_REGISTERS);
  if (status != No_Error)
    return status;
...
PZEM017::status_t PZEM017::set_high_voltage_alarm(float value) {
  ...
  if (!mbm->receive_response(response_buf, 8, 1000))
    return Timeout;
  PZEM017::status_t status = validate_response(8, WRITE_SINGLE_REGISTER);
  return status;
}
...
PZEM017::status_t PZEM017::set_low_voltage_alarm(float value) {
  uint16_t value_uint16 = value * 100.;
  mbm->send_message(address, WRITE_SINGLE_REGISTER, Low_Voltage_Alarm_Parameter,
                    value_uint16, 5, 5);
  if (!mbm->receive_response(response_buf, 8, 1000))
    return Timeout;
  PZEM017::status_t status = validate_response(8, WRITE_SINGLE_REGISTER);
...
PZEM017::status_t PZEM017::reset_energy() {
  mbm->send_message(address, (modbus_function_code_t)(Reset_Energy), 0, 0, 5,
                    5);
  if (!mbm->receive_response(response_buf, 8, 1000))
    return Timeout;
  PZEM017::status_t status = validate_response(8, (modbus_function_code_t)Reset_Energy);
  return status;
}
...
PZEM017::status_t PZEM017::validate_response(uint response_len,
