...
void app_entry()
{
    ...
    lv_theme_t * th =
        lv_theme_default_init(NULL,                                   /* Use DPI, size, etc. from this display */
...
void splash_screen(uint32_t delay)
{
    ...
    lv_anim_set_exec_cb(&a, [](void * obj, int32_t v) {
        ...
        if(v == 255) lv_obj_fade_out((lv_obj_t *)obj, splash_fadeout_dur, splash_stay_dur);
    });
...
void home_screen(uint32_t delay)
{
    ...
    lv_obj_set_style_pad_all(center_grid, 0, LV_PART_MAIN);    // Ensure no padding in the bottom_button_grid
    lv_obj_set_style_pad_row(center_grid, 0, LV_PART_MAIN);    // Remove row padding
    lv_obj_set_style_pad_column(center_grid, 0, LV_PART_MAIN); // Remove column padding
    ...
    lv_obj_set_size(left_ctr_grid, lv_pct(50), lv_pct(100)); // Set the size of the grid
    lv_obj_set_style_radius(left_ctr_grid, 0, LV_PART_MAIN); // Set border radius to 0
    lv_obj_set_style_border_width(left_ctr_grid, 0, LV_PART_MAIN);
    // lv_obj_set_style_bg_color(left_ctr_grid, lv_palette_main(LV_PALETTE_GREEN), 0); // Set background color to green
    lv_obj_set_style_pad_all(left_ctr_grid, 0, LV_PART_MAIN);    // Ensure no padding in the bottom_button_grid
    lv_obj_set_style_pad_row(left_ctr_grid, 0, LV_PART_MAIN);    // Remove row padding
    lv_obj_set_style_pad_column(left_ctr_grid, 0, LV_PART_MAIN); // Remove column padding
    lv_obj_align(left_ctr_grid, LV_ALIGN_LEFT_MID, 0, 0);

    // Set the flex layout for the left_ctr_grid to arrange children in rows
    lv_obj_set_flex_flow(left_ctr_grid, LV_FLEX_FLOW_COLUMN); // Arrange in a column (vertical)
    ...
    create_row_container(left_ctr_grid, 2, [](lv_obj_t * row_container) {
        ...
        lv_obj_set_style_text_font(big_label, &lv_font_montserrat_48, 0); // Use a large font for big text
        ...
        lv_obj_set_style_text_font(small_label, &lv_font_montserrat_16, 0); // Use a smaller font for small text
    });
    ...
    create_row_container(left_ctr_grid, 1, [](lv_obj_t * row_container) { // Set up the row container with flex layout
    ...
    lv_obj_set_size(right_ctr_grid, lv_pct(50), lv_pct(100)); // Set the size of the grid
    lv_obj_set_style_radius(right_ctr_grid, 0, LV_PART_MAIN); // Set border radius to 0
    ...
    lv_obj_set_style_pad_all(right_ctr_grid, 0, LV_PART_MAIN);    // Ensure no padding in the bottom_button_grid
    lv_obj_set_style_pad_row(right_ctr_grid, 0, LV_PART_MAIN);    // Remove row padding
    lv_obj_set_style_pad_column(right_ctr_grid, 0, LV_PART_MAIN); // Remove column padding
    lv_obj_align(right_ctr_grid, LV_ALIGN_RIGHT_MID, 0, 0);

    // Set the flex layout for the right_ctr_grid to arrange children in rows
    lv_obj_set_flex_flow(right_ctr_grid, LV_FLEX_FLOW_COLUMN); // Arrange in a column (vertical)
    ...
    create_row_container(right_ctr_grid, 2, [](lv_obj_t * row_container) {
        ...
        lv_obj_set_style_text_font(cutoff_label, &lv_font_montserrat_16, 0); // Use a smaller font for small text
    ...
    lv_obj_set_size(bottom_button_grid, lv_pct(100), bottom_grid_height); // Set the bottom_button_grid size
    lv_obj_set_style_radius(bottom_button_grid, 0, LV_PART_MAIN);
    lv_obj_set_style_pad_all(bottom_button_grid, 0, LV_PART_MAIN);    // Ensure no padding in the bottom_button_grid
    lv_obj_set_style_pad_row(bottom_button_grid, 0, LV_PART_MAIN);    // Remove row padding
    lv_obj_set_style_pad_column(bottom_button_grid, 0, LV_PART_MAIN); // Remove column padding

    lv_obj_align(bottom_button_grid, LV_ALIGN_BOTTOM_MID, 0, 0); // Align bottom_button_grid to bottom

    static lv_coord_t bottom_col_dsc[] = {LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_FR(1),
